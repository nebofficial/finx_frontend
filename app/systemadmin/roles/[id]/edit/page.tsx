...
import { ArrowLeft, Shield, Loader2, Save, FileEdit } from 'lucide-react'
...
export default function EditRolePage({ params }: { params: { id: string } }) {
  ...
  const [formData, setFormData] = useState({ name: 'TenantAdmin', description: 'Manage cooperative and view analytics', permissions: ['users:read', 'roles:read', 'billing:read', 'support:read'] })
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setIsLoading(true)
    try {
      const response = await fetch(`/api/admin/roles/${params.id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(formData) })
      if (response.ok) { toast.success('Role updated successfully'); router.push('/systemadmin/roles') } else { toast.error('Failed to update role') }
  ...
  return (
    <div className="space-y-6 min-h-screen p-6 rounded-xl" style={{ backgroundColor: THEME.mainBg }}>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 style={{ color: THEME.titleColor }} className="text-3xl font-bold tracking-tight">Edit Role</h1>
          <p style={{ color: THEME.subtitleColor }} className="mt-1 text-sm">
            Modify details and configure capability assignments.
          </p>
        </div>
      ...
      <form onSubmit={handleSubmit} className="grid grid-cols-1 xl:grid-cols-3 gap-6 items-start">
        ...
        <Card style={{ backgroundColor: THEME.cardBg, borderColor: THEME.border }} className="xl:col-span-2 shadow-sm border">
          <CardHeader style={{ borderBottom: `1px solid ${THEME.border}` }} className="rounded-t-2xl px-6 py-5">
            <div className="flex items-center gap-3">
              ...
              <div>
                <CardTitle style={{ color: THEME.titleColor }} className="text-lg font-bold">Role Definition</CardTitle>
                <CardDescription style={{ color: THEME.subtitleColor }} className="text-sm mt-0.5">
                  Update mapping and attributes.
                </CardDescription>
              </div>
            </div>
          </CardHeader>
          <CardContent className="p-6 space-y-8">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 p-5 rounded-2xl border shadow-sm" style={{ backgroundColor: THEME.mainBg, borderColor: THEME.border }}>
              <div className="space-y-2">
                ...
                <Input name="name" value={formData.name} onChange={handleChange} placeholder="e.g. Content Manager" style={inputStyle} className="text-sm shadow-sm h-11" />
              </div>
              <div className="space-y-2">
                <Label style={{ color: THEME.tableHeadText }} className="text-xs font-bold uppercase tracking-wide">Description</Label>
                <Input name="description" value={formData.description} onChange={handleChange} placeholder="Brief description of this role" style={inputStyle} className="text-sm shadow-sm h-11" />
              </div>
            </div>
            ...
            <div className="space-y-4">
               <div className="flex items-center justify-between pb-2 border-b" style={{ borderColor: THEME.border }}>
                <Label style={{ color: THEME.titleColor }} className="text-base font-bold">
                  Capability Matrix <span className="text-sm font-normal opacity-70 ml-2">({formData.permissions.length}/{allPermissions.length} selected)</span>
                </Label>
                    ...
                    <div key={group.group} style={{ backgroundColor: THEME.mainBg, borderColor: THEME.border }} className="rounded-xl border shadow-sm overflow-hidden p-0">
                      ...
                      <div className="p-3 space-y-1">
                        {group.items.map((perm) => (
                          <label key={perm.id} htmlFor={`edit-${perm.id}`} className="flex items-center gap-3 cursor-pointer group p-2 rounded-lg transition-colors"
                            style={{ backgroundColor: formData.permissions.includes(perm.id) ? THEME.hover : 'transparent' }}>
                            <Checkbox id={`edit-${perm.id}`} checked={formData.permissions.includes(perm.id)} onCheckedChange={() => handlePermissionToggle(perm.id)} />
        ...
        <div className="flex flex-col gap-5 sticky top-6">
          <Card style={{ backgroundColor: THEME.cardBg, borderColor: THEME.border }} className="shadow-sm border">
            <CardHeader style={{ borderBottom: `1px solid ${THEME.border}` }} className="rounded-t-2xl px-6 py-5">
              <CardTitle style={{ color: THEME.titleColor }} className="text-base font-bold">Actions</CardTitle>
            </CardHeader>
            <CardContent className="p-6 space-y-6">
              
              <div className="space-y-2">
                <Label style={{ color: THEME.titleColor }} className="text-xs font-bold uppercase tracking-wide px-1">Active Selections</Label>
                <div style={{ backgroundColor: THEME.mainBg, borderColor: THEME.border }} className="rounded-xl border p-4 space-y-2 max-h-[300px] overflow-y-auto shadow-inner">
                  {formData.permissions.length === 0 ? (
                    <p style={{ color: THEME.subtitleColor }} className="text-xs italic text-center py-4">No permissions mapped</p>
                  ) : (
                    formData.permissions.map((pid) => {
                      const perm = allPermissions.find((p) => p.id === pid)
                      return (
                        <div key={pid} className="flex items-center gap-2.5">
                          <div className="w-1.5 h-1.5 rounded-full flex-shrink-0" style={{ backgroundColor: THEME.titleColor }} />
                          <span style={{ color: THEME.subtitleColor }} className="text-[13px] font-medium leading-tight">{perm?.label ?? pid}</span>
                        </div>
                      )
                    })
                  )}
              ...
              <div className="space-y-3">
                <Button type="submit" disabled={isLoading} style={{ backgroundColor: THEME.titleColor, color: THEME.mainBg }}
                  ...
                  {isLoading ? <><Loader2 className="w-4 h-4 animate-spin" /> Storing…</> : <><Save className="w-5 h-5" /> Save Overrides</>}
                </Button>
